...
class Controller extends Library {
	...
	protected function parseParams($args, &$params, &$get = false, &$view = null) {
		...
		// O erro da passagem de parâmetros está aqui
		// $this->run(array('param' => 'value')) está virando $view em vez de $params
		if (isset($args[0]) && is_array($args[0]) && count($args[0]) == 1 && !is_numeric(key($args[0]))) {
			$view = array_shift($args);
		}
